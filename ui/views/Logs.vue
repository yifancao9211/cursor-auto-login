...
import { Trash2, Power, PowerOff } from "lucide-vue-next";
...
<template>
  <div class="flex flex-col gap-4 h-full max-w-5xl mx-auto">
    ...
    <div
      ref="logContainer"
      class="flex-1 min-h-0 overflow-y-auto bg-[#1e1e2e] rounded-xl border border-apple-border/30 shadow-inner"
      @scroll="handleScroll"
    >
      ...
      <table v-else class="w-full text-[12px] font-mono leading-relaxed">
        <tbody>
          ...
          >
            <td class="pl-3 pr-2 py-1 text-gray-500 whitespace-nowrap align-top select-none w-[70px]">{{ formatTime(entry.ts) }}</td>
