...
import { X, KeyRound, CheckCircle2, AlertCircle, PlayCircle, RotateCw } from "lucide-vue-next";
...
<template>
  <Transition name="modal">
    <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center no-drag p-4">
      <div class="absolute inset-0 bg-black/30 backdrop-blur-sm" @click="!running && (visible = false)"></div>
      ...
      <div class="relative bg-apple-bg w-full max-w-lg rounded-2xl shadow-apple-lg border border-white/50 flex flex-col overflow-hidden" 
        ...
        <div class="px-5 py-4 border-b border-apple-border flex justify-between items-center bg-white/40">
          <h3 class="font-bold text-apple-text">批量登录任务</h3>
          <button v-if="!running" class="w-7 h-7 flex items-center justify-center rounded-full bg-black/5 hover:bg-black/10 text-apple-textMuted transition-colors" @click="visible = false">
            <X class="w-4 h-4" />
          </button>
        </div>
        ...
        <div class="px-5 py-4 border-t border-apple-border flex justify-end items-center gap-3 bg-white/40">
          <button class="px-4 py-2 rounded-lg font-medium text-sm text-apple-textMuted hover:bg-black/5 transition-colors" @click="visible = false" :disabled="running">关闭面板</button>
